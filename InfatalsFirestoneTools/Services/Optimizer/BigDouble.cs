...
namespace BreakEternity
{
    ...
    public struct BigDouble : IComparable, IComparable<BigDouble>, IEquatable<BigDouble>
    {
        ...
        public static double powerOf10(int power)
        {
            ...
            // Faster AND simpler
            List<double> powersOf10 = [];

            for (long i = DOUBLE_EXP_MIN + 1; i <= DOUBLE_EXP_MAX; i++)
            {
                powersOf10.Add(double.Parse("1e" + i));
            }

            int indexOf0InPowersOf10 = 323;
        ...
        public static double f_magLog10(double n)
        {
        ...
        public static double f_gamma(double n)
        {
            ...
            if (n < -50)
            {
                return n == Math.Truncate(n) ? double.NegativeInfinity : 0;
            }
        ...
        private const double TWO_PI = 6.2831853071795864769252842; // 2*pi
        private const double EXP_N1 = 0.36787944117144232159553; // exp(-1)

        private const double OMEGA = 0.56714329040978387299997; // W(1, 0)
        ...
        public BigDouble d_lambertw(BigDouble z, double tol = 1e-10)
        {
            ...
            throw new ArithmeticException($"Iteration failed to converge: ${z}");
            //return dec.dNaN;
        }
        ...
        public double m
        {
                    ...
                    case 0:
                        {
                            ...
                            double man = mag == 5e-324 ? 5 : mag / powerOf10((int)exp);
                            return sign * man;
                        }
                    case 1:
                        {
            ...
            set
            {
                if (layer <= 2)
                {
                    _ = replaceFromMantissaExponent(value, e);
                }
                else
                {
   ...
   */
        ...
        private static BigDouble fromValueNoAlloc(string value)
        {
            ...
            return cached != null ? cached.Value : fromString(value);
        }
   ...
   */
        ...
        public static BigDouble randomBigDoubleForTesting(double maxLayers)
        {
            ...
            int randomsign = new Random().NextDouble() > 0.5 ? 1 : -1;
        ...
        public BigDouble replaceFromMantissaExponentNoNormalize(double mantissa, double exponent)
        {
            ...
            _ = replaceFromMantissaExponent(mantissa, exponent);
            return this;
        }
        ...
        public BigDouble replaceFromString(string value)
        {
            ...
            if (ecount == 0)
            {
                ...
                {
                    _ = replaceFromDouble(numberAttempt);
            ...
            if (newparts.Length == 2)
            {
                ...
                for (int i = 0; i < newparts[1].Length; ++i)
                {
                    ...
                    if (chrcode is (>= (char)43 and <= (char)57) or (char)101)
                    {
        ...
        public double mantissaWithdecPlaces(double places)
        {
            ...
            return double.IsNaN(m) ? double.NaN : m == 0 ? 0 : decPlaces(m, places);
        }

        public double magnitudeWithdecPlaces(double places)
        {
            // https://stackoverflow.com/a/37425022
            return double.IsNaN(mag) ? double.NaN : mag == 0 ? 0 : decPlaces(mag, places);
        }
        ...
        public override string ToString()
        {
            ...
            if (layer == 0)
            {
                return mag is (< 1e21 and > 1e-7) or 0 ? (sign * mag).ToString(CultureInfo.InvariantCulture) : m + "e" + e;
            }
            ...
            //layer 2+
            return layer <= MAX_ES_IN_A_ROW
                ? (sign == -1 ? "-" : "") + "e".repeat((int)layer) + mag
                : (sign == -1 ? "-" : "") + "(e^" + layer + ")" + mag;
        }

        public string toExponential(double places)
        {
            return layer == 0 ? (sign * mag).ToString("e" + places) : toStringWithdecPlaces(places);
        }

        public string toFixed(double places)
        {
            return layer == 0 ? (sign * mag).ToString("N" + places) : toStringWithdecPlaces(places);
        }

        public string toPrecision(double places)
        {
            return e <= -7 ? toExponential(places - 1) : places > e ? toFixed(places - exponent - 1) : toExponential(places - 1);
        }
        ...
        public string toStringWithdecPlaces(double places)
        {
            if (layer == 0)
            {
                return mag is (< 1e21 and > 1e-7) or 0
                    ? (sign * mag).ToString("N" + places)
                    : decPlaces(m, places) + "e" + decPlaces(e, places);
            }
            ...
            //layer 2+
            return layer <= MAX_ES_IN_A_ROW
                ? (sign == -1 ? "-" : "") + "e".repeat((int)layer) + decPlaces(mag, places)
                : (sign == -1 ? "-" : "") + "(e^" + layer + ")" + decPlaces(mag, places);
        }
        ...
        {
            return mag < 0 ? dZero : layer == 0 ? FC(sign, 0, Math.Round(mag)) : this;
        }

        public BigDouble floor()
        {
            return mag < 0 ? dZero : layer == 0 ? FC(sign, 0, Math.Floor(mag)) : this;
        }

        public BigDouble ceil()
        {
            return mag < 0 ? dZero : layer == 0 ? FC(sign, 0, Math.Ceiling(mag)) : this;
        }

        public BigDouble trunc()
        {
            return mag < 0 ? dZero : layer == 0 ? FC(sign, 0, Math.Truncate(mag)) : this;
        }
        ...
        {
            return mag == 0 ? dNaN : layer == 0 ? FC(sign, 0, 1 / mag) : FC(sign, layer, -mag);
        }
   ...
   */
        ...
        {
            return sign > value.sign ? 1 : sign < value.sign ? -1 : (int)sign * cmpabs(value);
        }
        ...
        public int cmpabs(BigDouble value)
        {
            ...
            return layera > layerb ? 1 : layera < layerb ? -1 : mag > value.mag ? 1 : mag < value.mag ? -1 : 0;
        }
   ...
   */
        ...
        {
            return sign == 0 ? dNaN : layer > 0 ? FC(Math.Sign(mag), layer - 1, Math.Abs(mag)) : FC(1, 0, Math.Log10(mag));
        }

        public BigDouble log10()
        {
            return sign <= 0 ? dNaN : layer > 0 ? FC(Math.Sign(mag), layer - 1, Math.Abs(mag)) : FC(sign, 0, Math.Log10(mag));
        }
        ...
        public BigDouble log(BigDouble b)
        {
            return sign <= 0
                ...
                : b.sign <= 0
                ? dNaN
                : b.sign == 1 && b.layer == 0 && b.mag == 1
                ? dNaN
                : layer == 0 && b.layer == 0 ? FC(this.sign, 0, Math.Log(this.mag) / Math.Log(b.mag)) : div(log10(), b.log10());
        }
        ...
        public BigDouble log2()
        {
            return sign <= 0
                ...
                : layer switch
                {
                    0 => FC(sign, 0, Math.Log2(mag)),
                    1 => FC(Math.Sign(mag), 0, Math.Abs(mag) * 3.321928094887362),//log2(10)
                    2 => FC(Math.Sign(mag), 1, Math.Abs(mag) + 0.5213902276543247),//-log10(log10(2))
                    _ => FC(Math.Sign(mag), layer - 1, Math.Abs(mag)),
                };
        }
        ...
        public BigDouble ln()
        {
            return sign <= 0
                ...
                : layer switch
                {
                    0 => FC(sign, 0, Math.Log(mag)),
                    1 => FC(Math.Sign(mag), 0, Math.Abs(mag) * 2.302585092994046),//ln(10)
                    2 => FC(Math.Sign(mag), 1, Math.Abs(mag) + 0.36221568869946325),//log10(log10(e))
                    _ => FC(Math.Sign(mag), layer - 1, Math.Abs(mag)),
                };
        }
        ...
        public BigDouble pow(BigDouble value)
        {
            ...
            return sign == -1 ? Math.Abs(b.toDouble() % 2) % 2 == 1 ? result.neg() : Math.Abs(b.toDouble() % 2) % 2 == 0 ? result : dNaN : result;
        }
        ...
        public BigDouble exp()
        {
            return mag < 0
                ...
                : layer switch
                {
                    0 when mag <= 709.7 => fromDouble(Math.Exp(sign * mag)),
                    0 => FC(1, 1, sign * Math.Log10(Math.E) * mag),
                    1 => FC(1, 2, sign * (Math.Log10(0.4342944819032518) + mag)),
                    _ => FC(1, layer + 1, sign * mag),
                };
        }
        ...
        public BigDouble slog(BigDouble? @base = null, int iterations = 100)
        {
            ...
            for (int i = 1; i < iterations; ++i)
            {
                ...
                bool currently_rose = new_dec.gt(this);
                if (i > 1)
                {
        ...
        public BigDouble slogTnternal(BigDouble? @base)
        {
            ...
            {
                int layerloss = (int)(copy.layer - b.layer - 3);
                result += layerloss;
        ...
        {
            //TODO: for bases above 10, revert to old linear approximation until I can think of something better
            return @base > 10 ? height - 1 : criticalSection(@base, height, criticalSlogValues);
        }
        ...
        public BigDouble layeradd(double diff, BigDouble @base)
        {
            ...
            return slogdest >= 0
                ? tetrate(@base, slogdest)
                : !double.IsFinite(slogdest)
                ? dNaN
                : slogdest >= -1 ? log(tetrate(@base, slogdest + 1), @base) : log(log(tetrate(@base, slogdest + 2), @base), @base);
        }
        ...
        {
            return lt(-0.3678794411710499)
                ? throw new ArgumentException("lambertw is unimplemented for results less than -1, sorry!")
                : mag < 0
                ? fromDouble(f_lambertw(this.toDouble()))
                : layer switch
                {
                    0 => fromDouble(f_lambertw(sign * mag)),
                    1 => d_lambertw(this),
                    2 => d_lambertw(this),
                    >= 3 => FC_NN(sign, layer - 1, mag),
                    _ => throw new ArgumentException("Unhandled behavior in lambertw()"),
                };
        }
        ...
        {
            return mag < 0 ? this : layer == 0 ? fromDouble(Math.Sin(sign * mag)) : FC_NN(0, 0, 0);
        }

        public BigDouble cos()
        {
            return mag < 0 ? dOne : layer == 0 ? fromDouble(Math.Cos(sign * mag)) : FC_NN(0, 0, 0);
        }

        public BigDouble tan()
        {
            return mag < 0 ? this : layer == 0 ? fromDouble(Math.Tan(sign * mag)) : FC_NN(0, 0, 0);
        }

        public BigDouble asin()
        {
            return mag < 0 ? this : layer == 0 ? fromDouble(Math.Asin(sign * mag)) : FC_NN(double.NaN, double.NaN, double.NaN);
        }

        public BigDouble acos()
        {
            return mag < 0
                ? fromDouble(Math.Acos(this.toDouble()))
                : layer == 0 ? fromDouble(Math.Acos(sign * mag)) : FC_NN(double.NaN, double.NaN, double.NaN);
        }

        public BigDouble atan()
        {
            return mag < 0 ? this : layer == 0 ? fromDouble(Math.Atan(sign * mag)) : fromDouble(Math.Atan(sign * 1.79e308d));
        }
        ...
        {
            return abs().gte(1) ? FC_NN(double.NaN, double.NaN, double.NaN) : ln(add(1).div(fromDouble(1).sub(this))).div(2);
        }
